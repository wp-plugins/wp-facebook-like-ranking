			...
			<tr>
				...
				<td>
					<select id="wp_fb_like_ranking_updatePostNumber" name="wp_fb_like_ranking_updatePostNumber">
						...
						<option value="10" <?php if ($WpFbLikeRankingUpdatePostNumber == 10) echo 'selected'; ?>>最新の10記事（10 latest posts）</option>
						<option value="20" <?php if ($WpFbLikeRankingUpdatePostNumber == 20) echo 'selected'; ?>>最新の20記事（20 latest posts）</option>
						<option value="30" <?php if ($WpFbLikeRankingUpdatePostNumber == 30) echo 'selected'; ?>>最新の30記事（30 latest posts）</option>
						<option value="40" <?php if ($WpFbLikeRankingUpdatePostNumber == 40) echo 'selected'; ?>>最新の40記事（40 latest posts）</option>
						<option value="50" <?php if ($WpFbLikeRankingUpdatePostNumber == 50) echo 'selected'; ?>>最新の50記事（50 latest posts）</option>
					</select>
				</td>
			</tr>
		</table>
